...
if('POST'== $_SERVER['REQUEST_METHOD'])  {
     ...
     if($_POST['add'] > 0) {
        ...
        $_SESSION['message'] = 'Sėkmingai pridėta '.$_POST['add'].' € !';
        file_put_contents(__DIR__ . '/bank.json', json_encode($data));
        header('Location: http://localhost/php/web_mechanics/bank/add_money.php?add='.$key);
        die;
...
<form class="addRemove" action="http://localhost/php/web_mechanics/bank/add_money.php?add=<?= $key ?>" method="post">  
...
<?php
        ...
        <br>
        <input type="text" placeholder="Pridėti" name="add">
        <div class="buttons">
        <a class="log" href = "http://localhost/php/web_mechanics/bank/list.php">Grįžti į sąrašą</a>
        <button class="log" type="submit" name="submit">Pridėti</button></div>';
